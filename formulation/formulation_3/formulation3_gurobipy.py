...
def build_model_from_definition(
    ...
    return Formulation3ModelBundle(
        model=model,
        variables={
            "z_b": z_b,
            "z_bq": z_bq,
...
def solve_problem(model: gp.Model | Formulation3ModelBundle) -> Formulation3Solution | None:
    if isinstance(model, Formulation3ModelBundle):
        bundle = model
        model = bundle.model
    else:
        bundle = None
