...
def _portable_index_tuple(key: Any, arity: int) -> tuple[int, ...]:
    ...
    if not isinstance(key, tuple):
        raise TypeError(
            f"expected tuple key for arity={arity}, got {type(key).__name__}"
        )
    if len(key) != arity:
...
def _portable_solution_payload(
    ...
    payload: dict[str, np.ndarray] = {
        ...
        "meta_backend_utf8": _encode_utf8_array(
            str(solution.meta.get("backend", "python"))
        ),
    ...
    for name in PORTABLE_VARIABLE_ARITIES:
        payload.update(
            _portable_variable_payload(name, solution.variables.get(name, {}))
        )
    return payload
...
def _load_portable_solution(path: Path) -> "Formulation3Solution":
    with np.load(path, allow_pickle=False) as data:
        if (
            ...
            != PORTABLE_SOLUTION_SCHEMA_VERSION
        ):
...
def _snapshot_tupledict(
    ...
    *,
    ...
    return {
        key: (float(var.X) if has_values else None) for key, var in variables.items()
    }
...
def coerce_reporting_inputs(
    ...
    if isinstance(prob, Formulation3Solution):
        wrapped_vars = {
            name: _SnapshotVarSet(values) for name, values in prob.variables.items()
        }
        return (
            _SnapshotModelView(
                status=prob.status,
