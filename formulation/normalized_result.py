...
from formulation.common.classes import Place, School
from formulation.common.utils import l_s
...
def _route_school_from_ordered_paths(
    ...
    ordered_paths: tuple[tuple[Place, Place], ...],
) -> School | None:
...
def normalized_result_from_formulation3_solution(
    ...
    for bus_idx, bus in enumerate(formulation.B):
        ...
        for round_idx, _round in enumerate(formulation.Q):
            if z_bq.get((bus_idx, round_idx), 0.0) <= 0.5:
                continue
            ordered_paths = _ordered_paths_for_round(
                formulation, solution, bus_idx, round_idx
            )
            if not ordered_paths:
                continue
            bus_distance += sum(formulation.d_ij(*path) for path in ordered_paths)

            served_schools = tuple(
                path[1] for path in ordered_paths if path[1] in formulation.S
            )
            school = served_schools[-1] if served_schools else None
            if school is None:
                continue
            stop_ids = tuple(
                str(path[0].name) for path in ordered_paths if path[0] in formulation.P
            )
            ...
            routes.append(
                NormalizedRoute(
                    ...
                    stop_ids=stop_ids,
                    distance_km=float(
                        sum(formulation.d_ij(*path) for path in ordered_paths)
                    ),
                    arrival_time_min=(
    ...
    return NormalizedRoutingResult(
        ...
        school_arrivals={
            key: values for key, values in school_arrivals.items() if values
        },
