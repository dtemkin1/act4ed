...
def _build_run_scopes(
    ...
    *,
    ...
    if not per_school and not per_school_type:
        return [
            RunScope(
                label_suffix="", description="all schools", problem_data=problem_data
            )
        ]

    student_counts_by_school = Counter(
        student.school.id for student in problem_data.students
    )
    if per_school:
        ...
        if not scopes:
            raise RuntimeError(
                "no schools with students were available for per-school runs"
            )
        return scopes

    student_counts_by_type = Counter(
        student.school.type for student in problem_data.students
    )
    scopes = [
        RunScope(
    ...
    if not scopes:
        raise RuntimeError(
            "no school types with students were available for per-school-type runs"
        )
    return scopes
...
def _scoped_label(base_label: str, scope: RunScope) -> str:
    return (
        base_label if scope.label_suffix == "" else f"{base_label}_{scope.label_suffix}"
    )
...
def _solve_with_python(
    ...
    *,
    ...
    if solution is None:
        raise RuntimeError(
            f"{label}: expected a Formulation3Solution from Python solve"
        )
...
def _solve_with_bird(
    ...
    *,
    ...
    _save_routing_solution_json(
        routing_solution_json, label=label, output_dir=output_dir
    )
...
def _run_case(
    *,
    ...
    if backend == "python":
        ...
        normalized_result = normalized_result_from_formulation3_solution(
